...
import { Folder, Layers, Search, ServerCrash, Sparkles } from 'lucide-react'
    ...
    <div className="space-y-6 p-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm uppercase tracking-[0.2em] text-muted-foreground">Organiza</p>
            <h1 className="text-2xl font-bold">Categorías de cursos</h1>
            <p className="text-sm text-muted-foreground">{filtered.length} resultado(s) • {categories.length} totales</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        <div className="flex items-center gap-3 rounded-xl border border-border bg-card/70 px-4 py-3 shadow-inner">
          ...
          <div>
            <p className="text-xs uppercase text-muted-foreground">Con descripción</p>
            <p className="text-lg font-semibold">{withDescription}</p>
          </div>
        </div>
        <div className="flex items-center gap-3 rounded-xl border border-border bg-card/70 px-4 py-3 shadow-inner">
          <Layers className="h-5 w-5 text-sky-600 dark:text-sky-300" />
          <div>
            <p className="text-xs uppercase text-muted-foreground">Filtradas</p>
      ...
      <Card>
        <CardHeader className="flex flex-col gap-4 border-b border-border bg-muted/40 pb-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <CardTitle className="text-xl">Áreas de conocimiento</CardTitle>
            <CardDescription>Explora las categorías disponibles para tus cursos.</CardDescription>
          </div>
          ...
          {isError ? (
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {filtered.map(category => (
                <Card key={category.id} className="border border-border bg-card/80 shadow-sm">
                  <CardContent className="space-y-3 p-4">
                    <div className="flex items-center justify-between">
                      <div className="text-xs uppercase tracking-wide text-muted-foreground">ID {category.id}</div>
                      <Badge variant="outline" className="text-xs font-medium">Activa</Badge>
                    </div>
                    <div>
